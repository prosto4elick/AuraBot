...
LAST_ANSWERS = {}
AURA_COOLDOWN = {}
USER_JOINS_TODAY = {} # Память для входов {user_id: {'count': 1, 'last_date': '2026-04-25'}}
...
# Варианты приветствий (первый раз)
WELCOME_VARIATIONS = [
...
# Варианты приветствий (перезаход)
REJOIN_VARIATIONS = [
...
LEAVE_VARIATIONS = [
    "Удачи, {name}!",
    "{name} покинул чат. Увидимся.",
    "До встречи, {name}.",
    "Минус один. Счастливо, {name}!"
]

# Прощание при частом выходе
...
async def welcome_new_member(message: types.Message):
    ...
    for user in message.new_chat_members:
        ...
        # Проверка на перезаходы за сегодня
...
async def goodbye_member(message: types.Message):
    ...
    # Если человек за день уже выходил/входил больше 1 раза
...
async def aura_luck(message: types.Message):
    question = "luck_check"; repeated = check_repeat(message.chat.id, question)
    if repeated: await message.reply(f"{random.choice(REPEAT_PHRASES)}<b>{repeated}</b>")
    else:
        luck = f"{random.randint(0, 100)}%"; save_answer(message.chat.id, question, luck)
...
async def aura_instant_value(message: types.Message):
    ...
    if uid in AURA_COOLDOWN and (now - AURA_COOLDOWN[uid]) < 10:
        left = int(10 - (now - AURA_COOLDOWN[uid])); await message.reply(f"⏳ Подожди {left} сек.")
        return
...
async def aura_random_num(message: types.Message):
    ...
    try:
        n1, n2 = int(parts[2]), int(parts[3]); res = random.randint(min(n1, n2), max(n1, n2))
        await message.reply(f"🔢 Число: <b>{res}</b>")
...
async def aura_timer(message: types.Message):
    parts = message.text.split()
    try:
        seconds = int(parts[2]); if seconds <= 0: raise ValueError
        await message.reply(f"⏳ Таймер на <b>{seconds}</b> сек."); await asyncio.sleep(seconds)
